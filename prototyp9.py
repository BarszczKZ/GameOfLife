...
def change_size_fun():
    global size, sqrt_size, btn, window, grid_window
    size = int(size_entry.get())**2
    sqrt_size=int(size**(1/2))
    btn.clear()
    grid_window.destroy()

clear_grid = Button(menu_frame,text="Clear grid", font=("Arial",20), width=18, height=3)
clear_grid.grid(sticky="w")

make_grid = Button(menu_frame,text="make grid", font=("Arial",20), width=18, height=3, command=make_grid_fun)
make_grid.grid(sticky="w")

change_size = Label(menu_frame, text="Change size of grid", font=("Arial",20))
...
start_stop = Button(menu_frame, text="Start", font=("Arial",20), width=18, height=3, command = test_fun)
